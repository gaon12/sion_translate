...
그중 **6개(고유 표현 3개 × 원문/정답 양쪽)는 containment 1.00** 입니다 —
challenge 문장이 코퍼스 행에 **통째로** 들어 있다는 뜻입니다.

- `김칫국부터 마시지 마.` — `data29.jsonl:185527` 의 `김칫국부터 마시지 마…`
  (말줄임표 하나 차이)
- `호랑이도 제 말 하면 온다더니.` — `data12.jsonl:97814` 의
  `으아, 깜짝이야! 호랑이도 제 말하면 온다더니…!`
- `え？今なんて言った？` — `data33.jsonl:178261` 의
  `…え？今なんて言った？取引開始してもう 15% 下がった？`

나머지 22개는 containment 0.62~0.89 로, 표현의 상당 부분이 코퍼스에 있습니다.
