...
사용법:
    ...
    2. sion-augment 실행 → data/bt_*.jsonl 로 합성쌍이 저장됩니다.
    3. 다음 sion-train 실행 때 자동 인식되어 재준비/학습에 반영됩니다.
...
def count_dataset_pairs(dataset_dir: Path, synthetic_prefix: str) -> tuple[int, int]:
    ...
    if not manifest_path.exists():
        raise FileNotFoundError(
            f"{dataset_dir} 에 준비된 데이터셋이 없습니다. 먼저 sion-train 을 실행하세요."
        )
...
def main() -> None:
    ...
    if total_written == 0:
        ...
        return
    log(
        f"완료: 총 {total_written:,}쌍. 다음 sion-train 실행 때 자동 인식되어 "
