"""번역 품질 평가 CLI — 고정 평가셋에서 chrF/BLEU 를 측정합니다.
    ...
    kjx-evaluate                          # 자체 test split 양방향 평가
    kjx-evaluate --benchmark flores.jsonl # 외부 벤치마크(JSONL) 평가
    kjx-evaluate --direction ko-ja \
      --compare deepl=deepl_out.txt \
      --compare google=google_out.txt     # 외부 서비스 출력과 비교

외부 서비스 비교 방법:
    1. kjx-evaluate --direction ko-ja --export-sources src.txt 로
       평가셋 원문을 파일로 뽑습니다.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--glossary",
        help="용어집 JSON 경로 (지정 시 kjx 번역에 용어 강제 적용; 기본: 설정의 data.glossary)",
    )
    parser.add_argument(
        "--no-glossary",
        action="store_true",
