...
사용자가 프로젝트 루트의 ``kjx.yaml`` 에 적은 값은 항상 자동값보다
우선합니다. 즉 kjx.yaml 은 '바꾸고 싶은 것만 적는 얇은 override 파일'입니다.
"""
...
from kjx.performance import available_cpu_count
...
def pick_batch_size(env: EnvironmentInfo, d_model: int) -> int:
    ...
    OOM 이 나면 kjx.yaml 의 training.batch_size_per_gpu 로 낮추면 됩니다.
    """
    if not env.cuda:
...
def apply_auto_settings(
    ...
    *,
    ...
    """사용자가 kjx.yaml 에 적지 않은 항목을 자동값으로 채웁니다.

    ``raw`` 는 kjx.yaml 을 그대로 읽은 dict 로, '어떤 키를 사용자가 직접
